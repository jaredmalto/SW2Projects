...
public class NaturalNumber3 extends NaturalNumberSecondary {
    ...
    public NaturalNumber3(int i) {
        ...
        if (i != 0) {
            //make rep equal to the i to string
            this.rep = Integer.toString(i);
        } else {
            /*
             * the user entering "0" can create problems if it is translated
    ...
    public NaturalNumber3(String s) {
        ...
        //if the user does not enter "0"
        if (!s.equals("0")) {
            //set the rep equal to the string since they are already the same type
            this.rep = s;
        } else {
            /*
             * the user entering "0" can create problems if it is translated
    ...
    public final int divideBy10() {
        ...
        //set the default last digit as 0
        String lastDigit = "0";

        int length = this.rep.length();
        //we only need to change things if the length is greater than 0
        if (length > 0) {
            /*
             * the last digit is the substring of the last character in the
             * string
             */
            lastDigit = this.rep.substring(this.rep.length() - 1,
                    this.rep.length());
            //take off the last digit of rep
    ...
    public final boolean isZero() {
        ...
        //abstract is 0 if the string's length is 0
